...
class DipolarField:
    ...
    def get_field_at_point(self, point) -> np.ndarray:
        ...
        term1 = np.array((3 * relative_loc * m_dot_r) / (rel_magnitude ** 5))
...
class DipolarFields:
    ...
    def atom_position_1d(self, dim="x") -> list:
        """
        Gets list of atom positions in single dimension

        :param string dim: coordinate plane to get positions at
    ...
    def get_field_at_point(self, point: np.ndarray) -> np.ndarray:
        """Gets resultant field from the Atom magmom at a given location
        B(r) = (mu_0/4pi) * [ ((3r(m dot r))/r^5) - (m/r^3) ]
                ^                   ^                   ^
                mag_perm            term1               term2
        :param point: 3d location to get resultant field vector at
        :returns: Magnetic field vector
        """
        B_field = np.empty(shape=3)
        for position, moment in zip(
            self.atoms.positions, self.atoms.get_initial_magnetic_moments()
        ):
            mag_perm = cnst.mu_0 / (4 * cnst.pi)
...
if __name__ == "__main__":
    ...
    fields = {
        "x": np.empty(shape=(nx, ny)),
        "y": np.empty(shape=(nx, ny)),
    ...
    for i, x_val in enumerate(x):
        for j, y_val in enumerate(y):
            (
                ...
                fields["y"][j][i],
                fields["z"][j][i],
            ) = multi_atoms.get_field_at_point([x_val, y_val, 0.1])
